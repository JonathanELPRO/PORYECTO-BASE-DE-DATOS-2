...
def realizarPedido(total):
    ...
    if request.method == 'POST':
        ...
        db.Pedidos.insert_one({"idCliente":ObjectId(idCliente), "fechaHora":fechaHora, "nota":nota, "tipoDePago":tipoDePago, "detallePedido":detallePedido, "montoTotal":montoTotal })
        db.Carrito.delete_one({"idCliente":ObjectId(idCliente)})
        
        return render_template('success.html',correoClienteRecibido=correoCliente)
...
def verPedido(_idSolicitado):
    correoCliente = str(session['usuario'])
    listaProductos = db.Pedidos.aggregate([{'$match': {"_id": ObjectId(_idSolicitado)}}
, {'$project': {'_id': 0, "detallePedido":1}}, 
